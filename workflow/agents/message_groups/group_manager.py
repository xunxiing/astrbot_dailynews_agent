...
class MessageGroupManager:
    ...
    async def build_report(
        ...
        *,
        ...
        llm_classify: Optional[LLMRunner],
        llm_write: Optional[LLMRunner],
        ...
        for it in items:
            ...
            if cid.suggested_new_tag:
                suggestions[cid.suggested_new_tag] = suggestions.get(cid.suggested_new_tag, 0) + 1
        ...
        if promote_new_tags and suggestions:
            try:
                changed = self._store.promote_learned_tags(suggestions=suggestions, min_count=promote_min_count)
                if changed:
                    astrbot_logger.info("[dailynews] tag store updated: %s", self._store.identifier)
            except Exception:
                astrbot_logger.warning("[dailynews] tag store promotion failed", exc_info=True)
    ...
    async def _write_markdown_with_llm(
        ...
        *,
            ...
            for e in entries[:30]:
                ...
                for it in items:
                    if str(it.get("source") or "") == e.source:
                        evidence = _strip_for_evidence(str(it.get("content") or ""), max_chars=1200)
                        break
                rows.append(
                    {
                        "tag": e.tag,
        ...
        system_prompt = str(load_template("templates/prompts/message_group_writer_system.txt") or "").strip()

        prompt = {"now": now_str, "groups": payload_groups, "max_bullets_per_tag": 6}

        try:
            md = await llm.ask(system_prompt=system_prompt, prompt=json.dumps(prompt, ensure_ascii=False))
        except Exception:
            astrbot_logger.warning("[dailynews] group mode LLM write failed", exc_info=True)
            return ""
