...
def _is_proxy_like(s: str) -> bool:
    x = (s or "").strip().lower()
    return x.startswith("socks5://") or x.startswith("socks5h://") or x.startswith("http://") or x.startswith(
        "https://"
...
def _is_twitter_url(s: str) -> bool:
    x = (s or "").strip().lower()
    return x.startswith("https://x.com/") or x.startswith("http://x.com/") or x.startswith(
        "https://twitter.com/"
...
async def fetch_latest_tweets(
    ...
    *,
    ...
    if not _is_twitter_url(url):
        raise ValueError("target_url must be an X/Twitter profile url like https://x.com/openai")
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            await page.wait_for_selector('article[data-testid="tweet"]', timeout=int(timeout_ms))
            ...
            for i in range(total):
                ...
                try:
                    pinned = (await t.locator('div[data-testid="socialContext"]').count()) > 0
                except Exception:
                    pinned = False
                if pinned:
                    continue
                ...
                try:
                    ...
                    img_urls = await images.evaluate_all("imgs => imgs.map(img => img.src)")
                    if not isinstance(img_urls, list):
                        img_urls = []
                    img_urls = [str(u) for u in img_urls if isinstance(u, str) and u.strip()]
                except Exception:
                    img_urls = []
            ...
            return [{"url": x.url, "text": x.text, "image_urls": x.image_urls} for x in out]
        finally:
            await browser.close()
